...
public class 最长不含重复字符的子字符串_68 {
	/*
	 * 题目描述
	输入一个字符串（只包含 a~z 的字符），求其最长不含重复字符的子字符串的长度。
	例如对于 arabcacfr，最长不含重复字符的子字符串为 acfr，长度为 4。
	
	思路:https://www.cnblogs.com/StrayWolf/p/6701197.html?utm_source=itdadao&utm_medium=referral
...
f(8) = 4,  acfr，因为d=7,f(7)=3,因此上一个r不在f(7)中，f(8) = f(7) + 1
	...
	public int longestSubStringWithoutDuplication(String str) {
	    ...
	    int maxLen = 0;  //最大长度
	    int mark=0;
	    int[] preIndexs = new int[256];//用于保存当前字母最近出现的位置
